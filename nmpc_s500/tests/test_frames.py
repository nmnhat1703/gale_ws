...
class TestQuaternionToEulerZYX(unittest.TestCase):
    ...
    def test_combined_yaw_and_roll_decodes_to_intrinsic_zyx(self):
        """Drone facing east (yaw=π/2), rolled right (roll=π/6), no pitch.

        Build the rotation directly as Rz(π/2) @ Ry(0) @ Rx(π/6), encode
        as quaternion, decode. Should recover [roll=π/6, pitch=0, yaw=π/2].
        """
        yaw, pitch, roll = np.pi / 2, 0.0, np.pi / 6
        R = Rotation.from_euler('ZYX', [yaw, pitch, roll]).as_matrix()
        q = Rotation.from_matrix(R).as_quat()
        rpy = quaternion_to_euler_zyx(*q)
        np.testing.assert_allclose(rpy, [roll, pitch, yaw], atol=1e-9)

    def test_combined_yaw_and_pitch_decodes_to_intrinsic_zyx(self):
        """Drone facing east (yaw=π/2), nose-up (pitch=π/9), no roll.

        Build as Rz(π/2) @ Ry(π/9) @ Rx(0). Should decode to
        [roll=0, pitch=π/9, yaw=π/2].
        """
        yaw, pitch, roll = np.pi / 2, np.pi / 9, 0.0
        R = Rotation.from_euler('ZYX', [yaw, pitch, roll]).as_matrix()
        q = Rotation.from_matrix(R).as_quat()
        rpy = quaternion_to_euler_zyx(*q)
        np.testing.assert_allclose(rpy, [roll, pitch, yaw], atol=1e-9)

    def test_combined_all_three_axes_decodes_to_intrinsic_zyx(self):
        """All three non-zero, non-degenerate.

        Build as Rz(0.7) @ Ry(0.3) @ Rx(-0.4). Should round-trip exactly.
        """
        yaw, pitch, roll = 0.7, 0.3, -0.4
        R = Rotation.from_euler('ZYX', [yaw, pitch, roll]).as_matrix()
        q = Rotation.from_matrix(R).as_quat()
        rpy = quaternion_to_euler_zyx(*q)
        np.testing.assert_allclose(rpy, [roll, pitch, yaw], atol=1e-9)
